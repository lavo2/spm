...
struct padded_pair {
	std::vector<float> V1;
	std::vector<float> V2;
	char padding[CACHE_LINE_SIZE-2*sizeof(std::vector<float>)];
};
...
int main(int argc, char *argv[]) {    
	...
	std::vector<padded_pair> pairs(omp_get_max_threads());
...
#pragma omp task firstprivate(size,x) shared(sum, pairs) untied
			{
				std::vector<float>& V1 = pairs[omp_get_thread_num()].V1;
				std::vector<float>& V2 = pairs[omp_get_thread_num()].V2;
				V1.reserve(size);
				V2.reserve(size);
