...
// Modified to spawn C++ threads instead of pthreads
// Original C code here:
// https://github.com/eliben/code-for-blog/blob/master/2018/clone/launch-benchmark.c
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (use_thread) {
		try {
			...
			fprintf(stderr, "thread joing error\n");
			exit(1);
		}
    } else {
      if (wait(NULL) == -1) {
